import { PrismaClient, SecurityScan as PrismaSecurityScan, SecurityScanType, ScanStatus } from '@prisma/client';
import { SecurityScan, ScanTarget, ScanResults, UserScanStats, ScannerLimits } from '../types/security';
...
export class SecurityScanRequestService {
  ...
  async updateScanStatus(
    ...
    if (additionalData?.results) {
      ...
      updateData.results = enrichedResults as any;
    }
   ...
   */
  async getScheduledScans(): Promise<SecurityScan[]> {
    const now = new Date();
   ...
   */
  async cleanupOldScans(daysOld: number = 90): Promise<number> {
   ...
   */
  private mapPrismaToSecurityScan(scan: PrismaSecurityScan): SecurityScan {
    return {
      id: scan.id,
      userId: scan.userId,
