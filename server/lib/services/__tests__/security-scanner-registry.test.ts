...
import { SecurityScanner } from '../../types/security';

describe('SecurityScannerRegistry', () => {
  let registry: SecurityScannerRegistry;
  ...
  describe('scanner management', () => {
    ...
    it('should get scanner by ID', () => {
      ...
      expect(scanner?.id).toBe('owasp-zap');
      expect(scanner?.name).toBe('OWASP ZAP');
    });
    ...
    it('should toggle scanner status', () => {
      ...
      expect(scannerAgain?.isActive).toBe(true);
    });
