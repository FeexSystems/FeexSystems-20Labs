...
async function discoverPinnedRepositories(): Promise<string[]> {
  ...
  const pinnedSection = html.match(/pinned-item-list[\\s\\S]{0,120000}/i)?.[0] || html;
...
function technologyCandidates(repo: GitHubRepo, files: TreeItem[], text: string) {
...
function id(prefix: string, value: string) { return `${prefix}_${createHmac("sha256", "feex-wm").update(value).digest("hex").slice(0, 24)}`; }
...
export async function syncPinnedProjects() {
  ...
  for (const name of names) {
    const repo = await github(`/repos/${ORG}/${name}`) as GitHubRepo;
    results.push(await syncRepository(repo));
  }
  return { organization: ORG, discovered: names, synced: results };
}

export async function syncRepository(repo: GitHubRepo, changedPaths?: string[]) {
  await ensureSchema();
  ...
  const tree = await github(`/repos/${repo.full_name}/git/trees/${repo.default_branch}?recursive=1`) as { tree: TreeItem[] };
  const files = tree.tree.filter((item) => item.type === "blob" && TEXT_EXTENSIONS.test(item.path)).slice(0, MAX_FILES);
  const targetPaths = changedPaths?.length ? new Set(changedPaths) : null;
  ...
  for (const file of selected) {
    ...
    if (file.path === "README.md" || file.path === "package.json") {
      try {
        const raw = await github(`/repos/${repo.full_name}/contents/${encodeURIComponent(file.path)}?ref=${repo.default_branch}`) as { content?: string; encoding?: string };
        if (raw.content) aggregate += Buffer.from(raw.content.replace(/\n/g, ""), "base64").toString("utf8");
      } catch { /* one inaccessible artifact must not abort the repository */ }
    }
  }
  ...
  await prisma.$executeRawUnsafe(`INSERT INTO world_model_events (id,project_id,event_type,changed_paths,payload) VALUES ($1,$2,$3,$4::jsonb,$5::jsonb)`, id("event", `${repo.full_name}:${Date.now()}`), projectId, changedPaths?.length ? "incremental_sync" : "initial_sync", JSON.stringify(artifacts), JSON.stringify({ repository: repo.full_name, commit: null }));
...
export async function retrieveWorld(query: string, limit = 12) {
  ...
  const projects = await prisma.$queryRawUnsafe(`SELECT id,name,full_name,description,url,language,last_synced_at FROM world_model_projects WHERE name ILIKE $1 OR full_name ILIKE $1 OR description ILIKE $1 ORDER BY last_synced_at DESC LIMIT $2`, q, limit) as unknown[];
  const technologies = await prisma.$queryRawUnsafe(`SELECT t.name, COUNT(*)::int AS project_count FROM world_model_technologies t JOIN world_model_relationships r ON r.target_id=t.id WHERE t.name ILIKE $1 GROUP BY t.name ORDER BY project_count DESC LIMIT $2`, q, limit) as unknown[];
  return { query, projects, technologies };
}

export function verifyGitHubSignature(rawBody: string, signature: string | undefined) {
