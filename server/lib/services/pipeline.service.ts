...
import { Pipeline, PipelineStage, PipelineTrigger, Deployment } from '../types/devops';
...
export class PipelineService extends EventEmitter {
  private runningPipelines = new Map<string, any>();
   ...
   */
  async getRepositoryPipelines(repositoryId: string, userId: string): Promise<Pipeline[]> {
    // Verify repository belongs to user
    const repositories = await repositoryService.getUserRepositories(userId);
    const repository = repositories.find(repo => repo.id === repositoryId);
   ...
   * Get pipeline by ID
   */
  async getPipeline(pipelineId: string, userId: string): Promise<Pipeline> {
   ...
   * Validate pipeline configuration
   */
  private validatePipelineConfig(config: {
    name: string;
    stages: PipelineStage[];
    triggers: PipelineTrigger[];
    environment?: Record<string, string>;
   ...
   * Add log entry
   */
  private addLog(context: any, level: string, message: string, stage?: string): void {
    const logEntry = {
   ...
   */
  private async updateDeploymentLogs(deploymentId: string, logs: any[]): Promise<void> {
    try {
      await prisma.deployment.update({
        where: { id: deploymentId },
        data: { logs },
   ...
   */
  private async stopPipelineExecution(deploymentId: string): Promise<void> {
    const context = this.runningPipelines.get(deploymentId);
   ...
   */
  private mapPipelineFromDb(pipeline: any): Pipeline {
    return {
      id: pipeline.id,
      repositoryId: pipeline.repositoryId,
      name: pipeline.name,
