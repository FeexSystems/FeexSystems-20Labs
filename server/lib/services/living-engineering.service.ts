...
const key = (prefix:string, value:string) => `${prefix}_${createHmac("sha256","feex-wm").update(value).digest("hex").slice(0,24)}`;
...
async function discoverPinnedRepositories() {
  ...
  const section = html.match(/pinned-item-list[\s\S]{0,120000}/i)?.[0] || "";
  const names = new Set<string>(); const re = new RegExp(`href=[\\\"']/${ORG}/([^\\\"']+)[\\\"']`,"g"); let m:RegExpExecArray|null;
  while((m=re.exec(section))) { const n=m[1].split("/")[0]; if(n && !n.includes("?") && n!=="repositories") names.add(n); }
  return [...names].slice(0,12);
}

function technologies(repo:Repo, files:TreeItem[], text:string) {
  const lower=text.toLowerCase(), out=new Set<string>(); if(repo.language) out.add(repo.language);
  const signatures:Record<string,string[]>={React:["react"],TypeScript:["typescript","tsconfig"],Vite:["vite"],"Next.js":["next"],"Node.js":["express","node"],PostgreSQL:["postgres","postgresql","prisma"],Prisma:["prisma"],Redis:["redis","ioredis"],Docker:["docker","dockerfile"],"Three.js":["three","react-three"],TailwindCSS:["tailwind"],Supabase:["supabase"],Python:["python","pyproject","requirements.txt"],Kubernetes:["kubernetes","helm","k8s"],GraphQL:["graphql"],WebSockets:["socket.io","websocket"]};
...
export async function syncPinnedProjects() {
  ...
  for(const name of names) synced.push(await syncRepository(await gh(`/repos/${ORG}/${name}`) as Repo));
  return {organization:ORG,discovered:names,synced};
}

export async function syncRepository(repo:Repo, changedPaths:string[]=[]){
  ...
  const tree=await gh(`/repos/${repo.full_name}/git/trees/${repo.default_branch}?recursive=1`) as {tree:TreeItem[]};
  const files=tree.tree.filter(x=>x.type==="blob"&&TEXT.test(x.path)).slice(0,MAX_FILES); const target=changedPaths.length?new Set(changedPaths):null; const selected=target?files.filter(f=>target.has(f.path)):files; let aggregate=`${repo.name} ${repo.description||""} ${repo.language||""} ${(repo.topics||[]).join(" ")}`; const artifactPaths:string[]=[];
  for(const file of selected){
    ...
    if(file.path==="README.md"||file.path==="package.json"){try{const raw=await gh(`/repos/${repo.full_name}/contents/${encodeURIComponent(file.path)}?ref=${repo.default_branch}`) as {content?:string}; if(raw.content) aggregate+=Buffer.from(raw.content.replace(/\n/g,""),"base64").toString("utf8");}catch{}}
  }
