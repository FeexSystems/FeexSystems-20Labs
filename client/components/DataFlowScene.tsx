    ...
    function animate() {
      ...
      serverNodes.forEach((server, index) => {
        ...
        server.children.slice(1).forEach((light, lightIndex) => {
          const lightMaterial = (light as THREE.Mesh)
            .material as THREE.MeshStandardMaterial;
          lightMaterial.emissiveIntensity =
            0.5 + Math.sin(time * 3 + lightIndex) * 0.3;
        });
      ...
      codeFragments.forEach((fragment) => {
        ...
        // Pulse opacity
        const material = (fragment.children[0] as THREE.Mesh)
          .material as THREE.MeshBasicMaterial;
        material.opacity =
          fragment.userData.originalOpacity +
      ...
      dashboardGroup.children.slice(1).forEach((metric, index) => {
        const height =
          metric.userData.baseHeight + Math.sin(time * 1.5 + index) * 0.3;
        metric.scale.y = height;

        const material = (metric as THREE.Mesh)
          .material as THREE.MeshStandardMaterial;
