...
export function computeForceLayout(
  ...
  type P = { id: string; x: number; y: number; z: number; vx: number; vy: number; vz: number; type: string };
  const particles: P[] = [];
  ...
  const pos = new Map(particles.map((p) => [p.id, [p.x, p.y, p.z] as [number, number, number]]));
  return nodes.map((n) => ({
    ...n,
